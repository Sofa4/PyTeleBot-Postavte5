...
class Users:
    ...
    def __str__(self):
        return f"Name user: {self.firstName}   " \
               f"id: {self.userName}   " \
               f"lang: {self.languageCode}"

    def getUserHTML(self):
        return f"Name user: {self.firstName}   " \
               f"id: <a href='https://t.me/{self.userName}'>{self.userName}</a>   " \
               f"lang: {self.languageCode}"
...
class Menu:
    ...
    @classmethod
    def getCurMenuName(cls, chat_id):
        return cls.cur_menu.get(chat_id).name


m_main = Menu("Главное меню", buttons=["Развлечения", "Игры", "Помощь"])

m_games = Menu("Игры", buttons=["Камень, ножницы, бумага", "Крестики-нолики", "Выход"], parent=m_main)
m_game_21 = Menu("Игра в 21", buttons=["Карту!", "Стоп!", "Выход"], parent=m_games, handler="game_21")
m_game_rsp = Menu("Камень, ножницы, бумага", buttons=["Выход"], parent=m_games, handler="game_rsp")
m_game_ttt = Menu("Крестики-нолики", buttons=["Новая игра", "Выход"], parent=m_games, handler="game_ttt")

m_DZ = Menu("ДЗ", buttons=["Задание-1", "Задание-2", "Задание-3", "Задание-4", "Задание-5", "Задание-6", "Выход"],
            parent=m_main)

m_fun = Menu("Развлечения", buttons=["Прислать собаку", "Прислать фильм",
                                     "Прислать кофе", "Прислать гороскоп", "Выход"], parent=m_main)
m_horoscope = Menu("Прислать гороскоп", buttons=["Выход"], parent=m_fun)
